...
> [!IMPORTANT]
> Kimi Nola is in active development. Windows x64 and ARM64 installers are available on GitHub Releases. `v0.1.2` is the first updater-enabled release; older installations require one manual upgrade to `v0.1.2` or later before future stable updates can arrive through the app.
...
- Tauri updater artifacts and an unsigned `latest.json` manifest containing verified installer signatures are generated by the tag workflow.
- Complete x64 and ARM64 portable archives are generated with the installers.
  Tag releases use SignPath for Authenticode before the updater signature is
  generated, and the exact downloaded installers and portable PE payloads have
  their architecture, hashes, and Authenticode signer re-verified before the
  manifest is uploaded. In-app update acceptance remains a post-publication
  release-readiness step because drafts are absent from the stable feed.
...
GitHub Actions first dispatches and verifies a separate physical x64 and ARM64 hardware gate, then
creates one draft GitHub Release. It builds NSIS installers, updater signatures,
and complete portable archives in parallel, verifies the exact uploaded assets,
and uploads one serialized `latest.json` manifest. Test the draft before
