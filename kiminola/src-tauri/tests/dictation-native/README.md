...
```text
cargo test --manifest-path tests/dictation-native/Cargo.toml -- --nocapture --test-threads=1
cargo clippy --manifest-path tests/dictation-native/Cargo.toml --all-targets -- -D warnings
cargo test --features tokio/test-util --lib dictation_native::tests -- --nocapture --test-threads=1
```

The standalone crate includes the production `dictation_native.rs`. It reexports the exact `global-hotkey` types that `tauri-plugin-global-shortcut` reexports, avoiding unrelated Tauri/ASR dependencies. The final command tests the actual application crate and real plugin import. `tokio/test-util` is needed by other dictation backend tests, not by the native module.
