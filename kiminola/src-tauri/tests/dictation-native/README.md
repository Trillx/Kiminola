...
The parent requests the queued trace after the original delivery outcome is fixed. Receiving the trace also confirms that a timed-out insertion has finished before diagnostic readback. Neither synchronization nor the later read can change the original outcome. Successful readback tests still require `Verified`, exactly one dispatch, exact content/caret and both preflight and post-dispatch guards. There is no insertion retry, clipboard access, foreground activation or input injection.

Two slow-dispatch regressions delay only the owned child's first `EM_REPLACESEL` by 300 ms. The readback fixture must still return `Verified` after exact production verification. The production-timeout test instead calls unchanged `message()` and must return `Uncertain` with Win32 error 1460, exactly one dispatch and no post-dispatch guard. It confirms the eventual exact insertion without upgrading that uncertain result. This separates a content-verification test from production timing policy rather than accepting either outcome in the same test.

Focused commands:

```text
cargo test --locked --lib dictation_native::tests::cross_process -- --nocapture --test-threads=1
cargo test --locked --manifest-path tests/dictation-native/Cargo.toml --lib dictation_native::tests::cross_process -- --nocapture --test-threads=1
```

Interpret the trace in sequence. `0xb1` sets the initial selection, `0xc2` is the single insertion, and each complete read consists of password character `0xd2`, text length `0xe`, text `0xd`, text length `0xe`, selection `0xb0`, and limit `0xd5`. The first two reads capture the initial state and production preflight. Reads after insertion in the trace are production verification. The explicitly labelled diagnostic read follows trace collection. A dispatch error isolates the send failure. A successful dispatch with missing/slow production read messages differs from a complete read with mismatched text, selection, style or limit. No post-dispatch guard call means production verification did not reach its successful predicate.

Hosted x64 run `35898256243` at `734d76a` isolated the failure. The synthetic insertion returned Win32 error 1460 after 209 ms; the child's native `EM_REPLACESEL` took 249 ms. Later text and caret were exact. Production correctly returned `Uncertain` because the send exceeded its 200 ms deadline. This proves a timing-policy mismatch in the successful-readback fixture, not why that runner's EDIT processing was slow.

Scratch-only probes also delayed the first post-dispatch read and changed the owned caret. Both failed the original `Verified` assertion with one insertion and distinct diagnostics. Retain any hosted failure's `owned editor` lines, native trace, architecture and token observations. Production timeout, security rules and the empty delivery allowlist remain unchanged.
...
- Actual lock/suspend, secure-desktop transitions, physical chord release, user-clipboard behavior and named application compatibility are not exercised by these fixtures. Local ARM64 and hosted x64 fixture results only cover their owned controls and synthetic events, not release qualification.
